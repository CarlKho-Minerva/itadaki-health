...
struct UpdateRequiredMessage: View {
  ...
  private var message: String {
    if showFirmwareUpdate && showDATAppUpdate {
      return "Your glasses firmware and app need updates before Itadaki DAT can start."
    }
    if showFirmwareUpdate {
      return "Your glasses firmware needs an update before Itadaki DAT can start."
    }
    return "The app on your glasses needs an update before Itadaki DAT can start."
  }
  ...
  var body: some View {
    VStack(spacing: 24) {
      ...
      VStack(spacing: 12) {
        TipItemView(
          resource: .videoIcon,
          text: "First, Itadaki DAT needs permission to use your glasses camera."
        )
        TipItemView(
          resource: .tapIcon,
